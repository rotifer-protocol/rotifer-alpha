...
const en = {
  ...
  tipBestFitness: "The highest fitness F(g) score ever recorded across all funds and all epochs. F(g) measures a gene's trading quality on a 0–1 scale, combining win rate, profit-loss ratio, and consistency.",
  tipFitness: "Fitness F(g) is the core performance metric of the Rotifer Protocol. It combines three dimensions — win rate, profit-loss ratio (R:R), and consistency — into a single 0–1 score. The PBT evolution algorithm uses F(g) to decide which funds need parameter mutation.",
...
const zh: Record<TranslationKey, string> = {
  ...
  tipBestFitness: "迄今为止所有基金 · 所有世代出现过的最高适应度分数。F(g) 综合胜率、盈亏比和一致性，在 0–1 区间衡量基因的交易质量。",
  tipFitness: "适应度 F(g) 是 Rotifer 协议衡量基因交易质量的核心指标，综合三个维度：胜率（Win Rate）、盈亏比（R:R Ratio）、一致性（Consistency），输出 0–1 范围内的单一分数。PBT 进化算法以 F(g) 为依据决定哪些基金需要参数变异。",
